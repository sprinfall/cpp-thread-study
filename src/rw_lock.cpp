...
public:
  ...
  int GetOrCreate(int key) {
    ...
    ObjectMap::iterator lb = object_map_.lower_bound(key);  // lb->first >= id
    if (lb != object_map_.end() && !(object_map_.key_comp()(key, lb->first))) {  // id >= lb->first
...
void TestMap() {
  ...
  //MapType::iterator lb = mymap.lower_bound(key);  // lb->first >= key
